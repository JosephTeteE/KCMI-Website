# KCMI Digital Platform — Architecture V1 Draft (historical)

**Status:** HISTORICAL — superseded for implementation references by [`ARCHITECTURE_V1.md`](ARCHITECTURE_V1.md) (**APPROVED — PHASE B FOUNDATION AUTHORIZED**).  
**Do not** treat this draft as the live architecture authority.
...
| `media_admin` | Programs, announcements, media, sermons — **no** pastoral narratives |
...
- Root may feature or redirect to current featured event
- `camp.kcmi-rcc.org` → redirect to applicable camp event (vanity/legacy)
...
Brand candidates `#eff5f5`, `#b60b13`, `#108c1d`, `#c298b7`, `#7c1963`: **VERIFIED** in legacy CSS; logo JPEG pixel match remains **EXTERNAL VERIFICATION REQUIRED** (not falsely claimed). Use primitive + semantic tokens; enforce contrast.
...
1. **Explicit Phase B build authorization** (initialize app) — documentation ADRs alone are not a deploy/init command.
2. **Featured-event behavior** at `events.kcmi-rcc.org/` (display vs redirect) — product choice within ADR-0002.
3. **Whether/when to introduce Proxy** — only if rewrite spike fails (document concrete gap).
...
## Appendix E — Phase B sufficiency statement

**Is the architecture internally sufficient to authorize Phase B foundation work without relying on unresolved external information?**

**Yes, for foundation scope:** app skeleton, Supabase Auth+MFA, RLS scaffolding, storage buckets, Turnstile integration points, security headers, CI, design tokens, and public-site shell — these do **not** require Google Forms schemas or live DNS cutover to begin in a **non-production** environment once **explicit Phase B implementation authorization** is given.
...
**This document alone does not initialize the application.** Explicit human instruction to start Phase B coding is still required under project change-control rules.

---

*End of Architecture V1 Draft (post–architecture-review update).*
