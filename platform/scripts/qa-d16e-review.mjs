/**
 * D1.6E review-only visual bundle from the local production build.
 * Writes platform/.qa-d16e-review/ (gitignored via .qa-*).
 ...
 *   node scripts/qa-d16e-review.mjs
 */
...
const OUT = resolve(ROOT, ".qa-d16e-review");
const SHOTS = resolve(OUT, "screenshots");
const PORT = process.env.QA_D16E_PORT || "3017";
const BASE = process.env.QA_D16E_BASE_URL || `http://127.0.0.1:${PORT}`;
...
function startServer() {
  if (process.env.QA_D16E_BASE_URL) return null;
...
async function enrollTotp(email) {
  ...
  const { data: enrolled, error: enrollErr } = await client.auth.mfa.enroll({
    factorType: "totp",
    friendlyName: `d16e-review-${Date.now()}`,
  });
...
<head>
  ...
  <title>KCMI D1.6E visual review (NOT APPROVED)</title>
  <style>
...
<body>
  <header class="banner">
    <h1>KCMI D1.6E visual review — not approved</h1>
    ...
    <p>Inventory: <strong>${entries.length}</strong> screenshots in <code>platform/.qa-d16e-review/</code></p>
  </header>
  <main class="wrap">
    <h2>Public routes</h2>
    ${publicHtml}
...
async function main() {
  ...
  try {
    ...
    if (process.env.QA_D16E_HUB_ONLY === "1" && existsSync(resolve(OUT, "metadata.json"))) {
    ...
    const superEmail = `d16e.review.sa.${stamp}@example.invalid`;
    const hqEmail = `d16e.review.hq.${stamp}@example.invalid`;
    ...
    await captureHubScreen(authPage, entries, {
      ...
      role: "Super Admin",
      description: "Dashboard — mobile Hub menu",
      file: "hub-dashboard-menu-390.png",
      prepare: async (page) => {
        await page.getByRole("button", { name: "Open Hub menu" }).click();
        await page.getByRole("dialog").getByRole("link", { name: "Dashboard" }).waitFor();
      },
    });
    ...
    await captureHubScreen(authPage, entries, {
      ...
      width: 1280,
      state: "section chooser",
      role: "Super Admin",
      description: "Homepage — What would you like to change?",
      file: "hub-home-chooser-1280.png",
    });
    await captureHubScreen(authPage, entries, {
      route: "/admin/website/home",
      width: 1280,
      ...
      role: "Super Admin",
      description: "Homepage — Top Banner editor",
      file: "hub-home-focused-1280.png",
      prepare: async (page) => {
    ...
    if (reviewProgramId) {
      await captureHubScreen(authPage, entries, {
        ...
        state: "draft",
        role: "Super Admin",
        description: "Program — Current draft",
      ...
      await captureHubScreen(authPage, entries, {
        ...
        state: "published",
        role: "Super Admin",
        description: "Program — Currently on the website",
    ...
    if (reviewSermonId) {
      await captureHubScreen(authPage, entries, {
        ...
        state: "draft",
        role: "Super Admin",
        description: "Sermon — Current draft",
      ...
      await captureHubScreen(authPage, entries, {
        ...
        state: "published",
        role: "Super Admin",
        description: "Sermon — Currently on the website",
    ...
    await captureHubScreen(authPage, entries, {
      ...
      state: "current",
      role: "Super Admin",
      description: "Branch editor — Current details",
      file: "hub-branch-editor-1280.png",
    });
    ...
    await captureHubScreen(authPage, entries, {
      ...
      state: "not live",
      role: "Super Admin",
      description: "Livestream — not live (no Turn off)",
    ...
    await captureHubScreen(authPage, entries, {
      ...
      state: "live",
      role: "Super Admin",
      description: "Livestream — live (synthetic Facebook fixture)",
    ...
    writeFileSync(
      ...
      [
        "KCMI D1.6E visual review — review only.",
        `Origin: ${BASE}`,
